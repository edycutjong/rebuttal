...
import type { Check, Claim, Resolved, Verdict, RebutEvent, AgentRun, CallEvent } from "@rebuttal/core";
...
export const EXAMPLES = [
  ...
  "Smart Money is buying $VVV on Base — net inflows all week",
  "A whale sold 600,000 UNI tokens, valued at approximately $5.1 million.",
];
...
export function rebuttalText(v: Verdict, permalink: string): string {
  ...
  return [
    `${v.label} — "${v.claim.raw.slice(0, 120)}${v.claim.raw.length > 120 ? "…" : ""}"`,
    `${tok}: ${v.reasons.join("; ")}.`,
    `Checked on Nansen: ${v.checks.filter((c) => c.ok).length}/${v.checks.length} calls, ${v.credits} credits · ${v.hash.slice(0, 12)} · ${permalink}`,
  ].join("\n");
}

type Phase = "idle" | "reading" | "checking" | "done" | "error";
type StreamEvent = RebutEvent | CallEvent | { type: "error"; message: string } | { type: "asOf"; asOf: string | null; degraded: boolean };
...
export function Rebuttal({
  ...
  prefill?: string;
  example: Verdict;
  exampleAgent?: AgentRun | null;
  proof: { tests: number; fixtures: number; p50: string; warm: string; credits: string };
}) {
  ...
  const rail = useRail(
    initialVerdict
      ? { calls: initialVerdict.provenance, label: initialVerdict.claim.raw, kind: "permalink", ms: initialVerdict.ms }
      : { calls: example.provenance, label: example.claim.raw, kind: "replayed", ms: example.ms },
  );
  const [symbols, setSymbols] = useState<Record<string, string>>(() => {
    const m: Record<string, string> = {};
    for (const v of [example, initialVerdict]) if (v?.resolved) m[shortAddr(v.resolved.address)] = v.resolved.symbol;
    return m;
  });
  ...
  const stream = useCallback(async (term: string) => {
    ...
    const group = rail.startGroup(term, "live");
      ...
      for (;;) {
        ...
        while ((nl = buf.indexOf("\n")) >= 0) {
          ...
          if (e.type === "input") setInput({ text: e.text, fromUrl: e.fromUrl, author: e.author });
            ...
            if (e.plan) setPlan(e.plan);
            if (e.resolved) setSymbols((m) => ({ ...m, [shortAddr(e.resolved!.address)]: e.resolved!.symbol }));
          } else if (e.type === "call") {
            rail.onCall(group, e);
    ...
    try {
      ...
      for (;;) {
        ...
        while ((nl = buf.indexOf("\n")) >= 0) {
          ...
          if (e.type === "tool_call") rail.agent.tool(group, e.name);
          else if (e.type === "done") finished = e.run;
  ...
  return (
    <>
        ...
        <div className="panel">
          ...
          >
            <input
              ...
              placeholder="paste the claim or the x.com link — “Smart Money is loading $X”"
              aria-label="claim text or x.com link"
              maxLength={600}
      ...
      {/* outside <main>: main.wrap is a size container (layout containment), which would pin a fixed toast to it instead of the viewport */}
      {toast && (
        <div className="toast" role="status">
          {toast}
        </div>
      )}
      <Rail rows={rail.rows} groups={rail.groups} symbols={symbols} onClear={rail.clear} onRunExample={() => run(example.claim.raw)} />
    </>
  );
}
