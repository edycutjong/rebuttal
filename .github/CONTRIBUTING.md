...
The web footer reads `package.json` at build time, so the deployed site shows the tag it was built from — after a release, redeploy (`vercel deploy --prod`) so the footer matches the latest tag.
