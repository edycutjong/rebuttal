...
| Route | What | Guard |
|---|---|---|
| `/` | the ONE flow; idle state shows the recorded hero fixture (0 credits, labelled). The **Nansen call rail** (`components/Rail.tsx`, right-hand panel ≥ 1280 px, bottom sheet below) streams every call from the client's `onCall` feed — `start` (pending row) → `end` (the same `Call` object that lands in provenance): endpoint, param summary, credits, ms, hash; counters for the current run, session totals, `clear`. On load it shows the fixture's calls labelled `replayed · 0 cr` | — |
| `/api/rebut?q=[&chain=][&stream=1]` | JSON verdict, or NDJSON `input · claim · resolved · call(start/end)×N · check×N · verdict · prose · asOf` | 400 on bad input before any fetch; 10 / IP / min → 429; 2,000 live credits / day then a labelled fixture replay or 503 |
