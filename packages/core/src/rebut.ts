...
export async function rebut(client: NansenClient, input: string, opts: RebutOptions = {}): Promise<Verdict> {
  ...
  const finish = (resolved: Resolved | null, d: Decision, evidence: Evidence, checks: Check[], proseP?: Promise<Prose>) => {
    return (async () => {
      const provenance = client.calls.slice(firstCall);
      const hash = verdictHash(claim, resolved, d, evidence);
      const prose = proseP ? await proseP : { text: templateProse(claim, resolved, d), source: "template" as const, ms: 0 };
      const v: Verdict = {
        input,
        claim,
        resolved,
  ...
  if (!validClaim(claim)) {
    ...
    const d = unverifiable(claim, claim.problem ?? "could not read a checkable claim", "U-CLAIM");
    const v = await finish(null, d, empty, []);
  ...
  // 6 · prose: the LLM paraphrases the decided record within its budget, else the template
  ...
  const v = await finish(resolved, d, evidence, checks, proseP);
  emit({ type: "verdict", verdict: v });
  emit({ type: "prose", prose: v.prose });
  return v;
}
