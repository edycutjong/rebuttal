...
![tests](https://img.shields.io/badge/tests-169%20passing-22c55e?style=flat)
![property cases](https://img.shields.io/badge/property_cases-20%2C000-22c55e?style=flat)
...
<img src="docs/screenshots/04-verdict.png" alt="Rebuttal, live: the claim card reads Smart Money buying PEPE on ethereum, six trace rows have landed, and the verdict card is red — CONTRADICTED, rule C-NOBODY: no Smart Money wallet traded PEPE in 24 h while 89 Smart Money traders sit on its table" width="100%">
...
| Tests | vitest + fast-check: 169 tests, 20,000 property cases, 10,000 generated bad inputs at the route boundary | the label is a pure function of the evidence |
...
| Metric | Value | Source |
|---|---|---|
| Tests | **169 tests** (`npm test`) | `packages/core/test/` |
| Property-based verification | **20,000 generated cases** (fast-check): `decide()` is pure, always one of four labels, selling is the exact mirror of buying; `extractClaim()` never throws on any string | `packages/core/test/property.test.ts` |
...
**Pipeline:** Quality (typecheck · 169 tests with coverage · offline replay · readiness) ∥ Security (TruffleHog on the full history · npm audit) → Build (Next.js, no key) → Deploy gate. No API key anywhere in CI.

```bash
npm run typecheck      # tsc strict, core + scripts + web
npm test               # 169 vitest tests incl. 20,000 property cases
