...
![tests](https://img.shields.io/badge/tests-168%20passing-22c55e?style=flat)
...
| Metric | Value | Source |
|---|---|---|
| Tests | **168 tests** (`npm test`) | `packages/core/test/` |
...
**Pipeline:** Quality (typecheck · 168 tests with coverage · offline replay · readiness) ∥ Security (TruffleHog on the full history · npm audit) → Build (Next.js, no key) → Deploy gate. No API key anywhere in CI.

```bash
npm run typecheck      # tsc strict, core + scripts + web
npm test               # 168 vitest tests incl. 20,000 property cases
